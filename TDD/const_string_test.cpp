...
{
	const_string cs0( ""); 
	BOOST_CHECK_EQUAL( cs0.length(), (size_t)0 );						// 1 //
	BOOST_CHECK( cs0.is_empty() ); 
	
	const_string cs01( NULL );
	BOOST_CHECK_EQUAL( cs01.length(), (size_t)0 ); 						// 2 //
	BOOST_CHECK( cs01.is_empty() );  
	
	const_string cs1( "test_string" ); 
	BOOST_CHECK_EQUAL( std::strcmp( cs1.data(), "test_string" ), 0 ); 	// 3 //
