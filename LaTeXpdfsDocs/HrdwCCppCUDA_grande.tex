...
\subsection{Amortized Time (e.g. dynamically resizing array "ArrayList" or maybe \texttt{std::vector})}
...
\begin{center}
	\begin{tabular}{ l | c | r }
		\hline
		Operation & Average & Amortized Worst Case \\ \hline
		Copy & $O(N)$ & $O(N)$ \\ \hline
...
\begin{center}
	\begin{tabular}{ l | c | c | c | c | p{50pt} }
		...
		insert/delete at end & $\begin{gathered} O(1) \text{ when last element is known } \\ O(N) \text{ when last element is unknown }\end{gathered}$ &  & $O(1)$ amortized  & $O(\log{N})$  & $O(N)$ \\ \hline 
